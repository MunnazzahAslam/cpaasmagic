                ...
                <Card>
                    <Accordion.Toggle as={Card.Header} eventKey={item.id}>
                        <div className='item-container' style={{ display: 'flex', flexDirection: 'row', alignItems: 'flex-start' }}>
                            ...
                            </div>{item.fields['API Logo'].map((c, i) => (
                                <div key={i}>
                                    <img style={{ margin: '0 2rem', borderRadius: '8px', width: '4vw', height: '4vw', marginLeft: '1rem' }} src={c.url} />
                                </div>
                            ))}
                            <div style={{ display: 'flex', flexDirection: 'column' }}>
                                <h2 style={{ fontSize: '1.3rem', color: '#222', fontWeight: '500' }}>{item.fields['Vendor Name']} - {item.fields['API Name']}</h2>
                                <p style={{ fontSize: '1.1rem' }}>{item.fields['API Type']}</p>
                            </div>
                        </div>
                    </Accordion.Toggle>
                    <Accordion.Collapse eventKey={item.id}>
                        <Card.Body>
                            ...
                            <p>{item.fields['API Description']}</p>
                            <p style={{ fontSize: '1.1rem', color: '#222', fontWeight: '500' }}>Features</p>
                            <p>{item.fields['API Feature 1']}</p>
                            <p>{item.fields['API Feature 2']}</p>
                            <p>{item.fields['API Feature 3']}</p>
                            <p>{item.fields['API Feature 4']}</p>
                            <p>{item.fields['API Feature 5']}</p>
                            <p>{item.fields['API Feature 6']}</p>
                            <p>{item.fields['API Feature 7']}</p>
                            <p>{item.fields['API Feature 8']}</p>
                            <p>{item.fields['API Feature 9']}</p>
                            <p style={{paddingBottom:'0.6rem'}}>{item.fields.API_Feature_10}</p>
                            <a href={item.fields['API Page'].url} style={{marginTop:'1rem'}}><button className='btn' style={{background:'#1283DA', marginTop:'2rem', color:'#fff', margin: '0 auto', borderRadius:'8px'}}>Learn More</button></a>
                        </Card.Body>
                    </Accordion.Collapse>
                </Card>
            </Accordion>
